...
<body>
    <div class="incidentReports">
        <h1>被害報告</h1>
            ...
            <h2 class="date">[被害に遭った日付：{{ $report->date }} 時間帯：{{ $report->timePeriod->time_slot}}]</h2>
                <h3>詳細</h3>
    ...
    <div class="suspiciousReports">
        <h1>不審者・不審物情報</h1>
            ...
            <h2 class="date">[目撃した日付：{{ $report->date }} 時間帯：{{ $report->timePeriod->time_slot}}]</h2>
                <h3>詳細</h3>
                <p>{{ $report->description }}</p>
            </div>
            @endforeach
    ...
    <div class="safetyReports">
        <h1>安全な駐輪場の共有</h1>
            ...
            <h2 class="date">[調査日：{{ $report->date }} 時間帯：{{ $report->timePeriod->time_slot}}]</h2>
                <h3>詳細</h3>
                <p>{{ $report->description }}</p>
            </div>
            @endforeach
